...
with open('host_lg.log') as file:
    for f in file:
        fileList.append(f)

with open('host_lg.log', 'w+') as file:
    for i in hosts:
...
Этот скрипт проверяет соответствие IP-адреса каждого из трех имен хостов, указанных в списке hosts, в файле 'host_lg.log'. В начале скрипт открывает файл 'host_lg.log' и добавляет каждую строку файла в список fileList. Затем скрипт проходит по каждому элементу списка hosts и получает IP-адрес хоста с помощью функции gethostbyname из модуля socket. Далее происходит проверка соответствия IP-адреса хоста с IP-адресами, уже записанными в файл 'host_lg.log'. Если IP-адрес найден в файле, то скрипт проверяет соответствие IP-адресов. Если IP-адреса совпадают, то скрипт выводит информацию о правильном соответствии в консоль и записывает эту информацию в файл 'host_lg.log'. Если IP-адреса не совпадают, то скрипт выводит сообщение об ошибке в консоль и записывает эту информацию в файл 'host_lg.log'. Если IP-адреса нет в файле 'host_lg.log', то скрипт выводит информацию о хосте и его IP-адресе в консоль и записывает эту информацию в файл 'host_lg.log'. В результате работы скрипта в файле 'host_lg.log' будут записаны соответствующие сообщения о соответствии или несоответствии IP-адресов каждого из трех хостов.
