...
# Команда `[[ -d '/tmp1' ]]` проверяет, существует ли директория `/tmp1`. Символы `[[` и `]]` используются в языке bash для выполнения условных выражений. Опция `-d` используется для проверки, является ли переданный путь директорией. 
