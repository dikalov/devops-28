...
Данный скрипт выполняет мониторинг изменений IP-адресов для списка сайтов. В начале скрипта определяется список сайтов, IP-адреса которых необходимо отслеживать, после чего происходит вызов функции check_list_of_sites, которая проверяет DNS для каждого сайта из списка и возвращает словарь со списком их IP-адресов. Функция check_site_dns используется для определения IP-адреса каждого сайта и сравнения его с предыдущим значением. Если IP-адрес для сайта изменился, то скрипт выводит сообщение об ошибке и обновляет словарь с новым значением IP-адресов. Мониторинг происходит каждые 5 секунд и продолжается бесконечно. Словарь с IP-адресами сайтов записывается в файлы JSON и YAML — ip.json и ip.yaml, соответственно.
