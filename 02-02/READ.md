...
# При использовании команды `ssh` без опции `-t` или `-tt`, которая указывает на выделение TTY (терминального устройства) для удаленного подключения, `ssh` не выделяет терминал. В данном случае, когда выполняется команда `ssh localhost 'tty'`, `ssh` не получает TTY, и поэтому возвращает сообщение "not a tty". Чтобы изменить это поведение и выделить TTY, можно использовать опцию `-t` или `-tt` при подключении по SSH. Например, команда `ssh -t localhost 'tty'` выделит TTY и вернет имя терминала.
#
# Задание 13
...
# Команда `sudo echo string > /root/new_file` не будет работать, потому что перенаправление `>` выполняется процессом shell без прав суперпользователя. Однако, когда команда `sudo tee` используется вместо `sudo echo`, команда `tee` выполняется с правами суперпользователя, что позволяет успешно выполнить операцию записи в файл.
